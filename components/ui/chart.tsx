...
// A permissive shape for tooltip items so we can access fields freely.
type TooltipItem = {
...
>(
  (
    ...
    const tooltipLabel = React.useMemo(() => {
      ...
      const [item] = payload
      const key = `${labelKey || (item as TooltipItem).dataKey || (item as TooltipItem).name || "value"}`
      ...
      >
        ...
        <div className="grid gap-1.5">
          {payload.map((item, index) => {
            const key = `${nameKey || (item as TooltipItem).name || (item as TooltipItem).dataKey || "value"}`
            const itemConfig = getPayloadConfigFromPayload(config, item, key)
            const indicatorColor = color || (item as TooltipItem).payload?.fill || (item as TooltipItem).color

            return (
              <div
                key={(item as TooltipItem).dataKey}
                className={cn(
                ...
                {formatter && (item as TooltipItem)?.value !== undefined && (item as TooltipItem).name ? (
                  ...
                  <>
                    ...
                    >
                      <div className="grid gap-1.5">
                        {nestLabel ? tooltipLabel : null}
                        <span className="text-muted-foreground">{itemConfig?.label || (item as TooltipItem).name}</span>
                      </div>
                      {(item as TooltipItem).value !== undefined && (
                        <span className="font-mono font-medium tabular-nums text-foreground">
                          {(item as TooltipItem).value!.toLocaleString()}
                        </span>
                      )}
                    </div>
                  </>
...
const ChartLegendContent = React.forwardRef<HTMLDivElement, React.ComponentProps<"div"> & LegendExtraProps>(
